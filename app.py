...
def calculate_subject_stats(subj, cfg, absent_records):
    ...
    is_at_risk = absences > max_allowed_absences
    safe_absences_left = max(0, max_allowed_absences - absences)
...
def main_app():
    ...
    if not cfg.get("setup_complete", False) or nav_mode == "⚙️ Timetable Setup":
        ...
        with tab_cancel:
            ...
            c_date = st.date_input("Select Date to Cancel:", min_value=sem_data.get("start_date", datetime.date.today()), max_value=sem_data.get("end_date", datetime.date.today() + datetime.timedelta(days=120)), key="c_date_add")
            ...
            if not cancelled_list:
                ...
                for idx, item in enumerate(cancelled_list):
                    ...
                    if col_btn.button("🗑️ Remove", key=f"del_can_{idx}"):
        ...
        with tab_extra:
            ...
            e_subj = st.selectbox("Select Subject:", options=all_subjects if all_subjects else ["Add subjects first"], key="e_subj_add")
            e_date = st.date_input("Select Date:", min_value=sem_data.get("start_date", datetime.date.today()), max_value=sem_data.get("end_date", datetime.date.today() + datetime.timedelta(days=120)), key="e_date_add")
            e_st = mobile_time_picker("Start Time", key_prefix="e_st_add")
            e_et = mobile_time_picker("End Time", key_prefix="e_et_add")

            if st.button("➕ Add Extra Class", type="primary") and all_subjects:
            ...
            if not current_extras:
                ...
                for idx, item in enumerate(current_extras):
                    ...
                    if col_btn.button("🗑️ Remove", key=f"del_ext_{idx}"):
